...
class Tailedi18n {
    ...
    constructor (params) {
        if (!params.list) {
            throw new Error('No list included!')
        }
        if (!fs.existsSync(params.list)) {
            throw new Error('Folder doesn\'t exist!')
        }
        if (!params.default && !params.disableWarnings) {
            console.log('You forgot to put default language! Are you sure?')
        }
        ...
        for (let i of fs.readdirSync(params.list)) {
            const localeFile = fs.readFileSync(params.list + i, 'utf8')
            this.locales[i.split(".")[0]] = JSON.parse(localeFile)
        }

        if (!this.locales[this.default] && this.default && !params.disableWarnings) {
            console.log(`Default language that you passed to the constructor (${this.default}) doesn't exist.`)
        }
    ...
    get (string) {
        if(!this.lang) {
            throw new Error('You forgot to choose language! Use Tailedi18n.select(lang) function.')
        }
        ...
        if (!output && this.default) {
            output = this.getDefault(args)
        }
    ...
    getDefault (args) {
        ...
        if (!this.disableWarnings) {
            if (!output) {
                console.log('String not found in both languages. Maybe you filled strings incorrectly?')
            } else {
                console.log(`String wasn't found in ${this.setLanguage}.json strings.`)
            }
        }
    ...
    declNumb (number, args, includeNumbers = true) {
        ...
        let number = Math.abs(number) % 100;
